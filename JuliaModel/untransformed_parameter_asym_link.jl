...
function fc_latent_sd(latent_sd, beta, X, gp_param, prior_sd = 1, lwr_lim = -Inf, upr_lim = Inf)
  if any(latent_sd .<= 0)
    return(-Inf)
  end
...
function fc_obs_sd(obs_sd, asym, offset, growth, dat, prior_sd = 1)
    out = 0.0
...
function draw_asym!(x0, dat, σ=0.1, track_accept = false)
    ...
    f0 = fc_gparam(x0[:asym], x0[:asym], x0[:offset], x0[:growth], x0[:obs_sd], 
                   x0[:beta_asym], x0[:X_asym], x0[:latent_sd_asym], dat, log(18000), log(22000), 0, Inf)

    f1 = fc_gparam(proposed, x0[:asym], x0[:offset], x0[:growth], x0[:obs_sd], 
                    x0[:beta_asym], x0[:X_asym], x0[:latent_sd_asym], dat,  log(18000), log(22000), 0, Inf)
    # Symmetric 
    a = (f1-f0)              
...
function draw_offset!(x0, dat, σ=0.1, track_accept = false)
    ...
    f0 = fc_gparam(x0[:offset], x0[:asym], x0[:offset], x0[:growth], x0[:obs_sd], 
                   x0[:beta_offset], x0[:X_offset], x0[:latent_sd_offset], dat, log(1.5), log(2.5), 0, Inf)
...
function draw_growth!(x0, dat, σ=0.1, track_accept = false)
    ...
    f0 = fc_gparam(x0[:growth], x0[:asym], x0[:offset], x0[:growth], x0[:obs_sd], 
                   x0[:beta_growth], x0[:X_growth], x0[:latent_sd_growth], dat, logit(0.75), logit(0.9), 0, 1)
...
function draw_latent_sd_asym!(x0, σ = 0.01, track_accept = false)
    ...
    f0 = fc_latent_sd(x0[:latent_sd_asym], x0[:beta_asym], x0[:X_asym], x0[:asym], 1000, 0, Inf)
    f1 = fc_latent_sd(prop, x0[:beta_asym], x0[:X_asym], x0[:asym], 1000, 0, Inf)
    a = (f1-f0)              
    if (log(rand(Uniform(0,1),1)[1]) < a )
...
function draw_latent_sd_offset!(x0, σ = 0.01, track_accept = false)
    ...
    ##fc_latent_sd(latent_sd, beta, X, outcome, prior_sd = 1)
    f0 = fc_latent_sd(x0[:latent_sd_offset], x0[:beta_offset], x0[:X_offset], x0[:offset],  0.3, 0, Inf)
    f1 = fc_latent_sd(prop, x0[:beta_offset], x0[:X_offset], x0[:offset], 0.3, 0, Inf)
    a = (f1-f0)              
    if (log(rand(Uniform(0,1),1)[1]) < a )
...
function draw_latent_sd_growth!(x0, σ = 0.01, track_accept = false)
    ...
    ##fc_latent_sd(latent_sd, beta, X, outcome, prior_sd = 1)
    f0 = fc_latent_sd(x0[:latent_sd_growth], x0[:beta_growth], x0[:X_growth], x0[:growth], 0.05, 0, 1)
    f1 = fc_latent_sd(prop, x0[:beta_growth], x0[:X_growth], x0[:growth], 0.05, 0, 1)
    a = (f1-f0)              
    if (log(rand(Uniform(0,1),1)[1]) < a )
...
function draw_obs_sd!(x0, dat, σ = 0.1, track_accept = false)
    ...
    f0 = fc_obs_sd(x0[:obs_sd], x0[:asym], x0[:offset], x0[:growth], dat, 10)
    f1 = fc_obs_sd(prop, x0[:asym], x0[:offset], x0[:growth], dat, 10)
    a = (f1-f0)              
    if (log(rand(Uniform(0,1),1)[1]) < a )
        x0[:obs_sd] = prop
        if track_accept
...
function run_mcmc_chain(fname, x0, monitors, dat, warmup, run, thin=10, verbose = 1)
    ...
    if (!isfinite(fc_latent_sd(x0[:latent_sd_asym], x0[:beta_asym], x0[:X_asym], x0[:asym], 1, 0, Inf)))
        error("asym latent sd full conditional not finite at init")
    end

    if (!isfinite(fc_latent_sd(x0[:latent_sd_offset], x0[:beta_offset], x0[:X_offset], x0[:offset], 1, 0, Inf)))
        error("offset latent sd full conditional not finite at init")
    end

    if (!isfinite(fc_latent_sd(x0[:latent_sd_growth], x0[:beta_growth], x0[:X_growth], x0[:growth], 1, 0, 1)))
    ...
    perturb_sd = Dict(:asym => 10, #asym
                 :offset => 0.01, #offset
                 :growth => 0.01, #growth
                 :beta_asym => 10, #beta_asym
                 :beta_offset => 0.04,  #beta_offset
                 :beta_growth => 0.04, #beta_growth
                 :latent_sd_asym => 10, #latent_sd_asym
                 :latent_sd_offset => 0.015, #latent_sd_offset
                 :latent_sd_growth => 0.015, #latent_sd_growth
                 :obs_sd => 5 #obs_sd
    )
